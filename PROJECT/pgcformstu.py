...
def assigncourse(cnm):
    global crsnm
    messagebox.showinfo("SUCCESS2",crsnm)
    crsnm=cnm
    messagebox.showinfo("SUCCESS3",crsnm)
...
def showreport(dnm,rl):

    top=Tk()
    top.title("ADMISSION REPORT")    
    
    srl=str(rl)
    rform= Label(top,text= "ADMISSION REPORT ").grid(row=0,column=1)
    d1 = Label(top,text="Department").grid(row =1, column = 0)
    '''Combobox in Course'''
    vd1 = Label(top,text="*   *")
    vd1.grid(row =1, column = 1)

    rl = Label(top,text="Rollnumber").grid(row =2, column = 0)
    vrl = Label(top,text="*  *")
    vrl.grid(row =2, column = 1)


    rnm = Label(top,text="Name").grid(row = 3, column = 0)
    vrnm = Label(top,text="*   *")
    vrnm.grid(row = 3, column = 1)

  
    rag=Label(top,text="Age").grid(row= 4,column=0)
    vrag=Label(top,text="*  *")
    vrag.grid(row= 4,column=1)


    rco=Label(top,text="Course").grid(row=5,column=0)
    vrco=Label(top,text="*  *")
    vrco.grid(row=5,column=1)



    rfe=Label(top,text="Fee").grid(row=6,column=0)
    vrfe=Label(top,text="*  *")
    vrfe.grid(row=6,column=1)

    db = mysql.connector.connect(host ="localhost",user = "root",password = "",db ="pgcdharamshala")
    qry="select * from stuinfo where sroll='"+srl+"' and Departmentname='"+dnm+"'" 
...
courses = ttk.Combobox(top, width = 20)
courses['values'] = cn
...
clr=Button(top,text="Clear",command=clear).grid(row=7,column=2)
